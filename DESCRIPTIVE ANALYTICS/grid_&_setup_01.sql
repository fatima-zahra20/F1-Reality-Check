...
WHERE s.year = 2023 AND s.session_name = 'Sprint Qualifying' AND g.lap_duration IS NOT NULL -- not dns 
...
SELECT s.year,g.session_key ,--rc.flag, 
	   m.meeting_name, s.session_name,g.driver_number, d.full_name, g."position", g.lap_duration, sr.dns
FROM silver_starting_grid g
INNER JOIN silver_drivers d 
    ON g.session_key = d.session_key AND g.driver_number = d.driver_number
JOIN silver_sessions s ON g.session_key = s.session_key
...
--JOIN silver_race_control rc ON s.session_key  = rc.session_key
WHERE s.year = 2023 
...
-- RACE QUALIFYING 
SELECT s.year ,m.meeting_name, s.session_name, g.driver_number, d.full_name, g."position", g.lap_duration
FROM silver_starting_grid g
INNER JOIN silver_drivers d 
    ON g.session_key = d.session_key AND g.driver_number = d.driver_number
JOIN silver_sessions s ON g.session_key = s.session_key
...
WHERE s.year = 2023 AND s.session_name = 'Qualifying' AND g.lap_duration IS NOT NULL  -- not dns 
	--AND m.meeting_name='Belgian Grand Prix'
	--AND s.session_key = 9135
...
--The Result: The penalty dropped his starting position down from 1st to 6th on the grid for Belgian Grand Prix, while Charles Leclerc inherited the official pole position start.
...
JOIN silver_session_result sr 
    ON s.session_key = sr.session_key AND g.driver_number = sr.driver_number
WHERE s.year = 2023 AND s.session_name = 'Qualifying' 
  --AND rc.flag = 'RED'
  --AND sr.dns = 0
  AND g.lap_duration IS  NULL
  ORDER BY g.lap_duration ASC
